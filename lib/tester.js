...
var installBookJson = function(bookPath, bookJson, modules, withLocalPlugins) {
  ...
  if(!withLocalPlugins && typeof book.plugins !== 'undefined' && modules.length > 0) {
    // remove all plugins locally installed plugins from book.json (they would be installed from NPM instead of
    // symlinked.
    book.plugins = book.plugins.filter(function(plugin) {
...
var execute = function(htmlContent, bookJson, children, localModules) {
  ...
  return createBook(htmlContent, bookJson, children)
    ...
    .then(function(bookPath){return installBookJson(bookPath, bookJson, modules, false);})
    .then(install)
    .then(function(bookPath){return installBookJson(bookPath, bookJson, modules, true);})
    .then(build)
    .then(readPages)
    .then(parsePages);
};
